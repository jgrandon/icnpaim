...
// Endpoints REST personalizados con logging
...
function lti_debug_communication_endpoint($request) {
    ...
    lti_log("Communication Debug", array(
...
function lti_log_all_rest_requests($response, $handler, $request) {
    ...
    // Solo loggear rutas relacionadas con nuestros CPTs
    if (preg_match('/\/(student|course|unit|progress|grade)/', $route)) {
...
function lti_register_connectivity_endpoints() {
...
add_action('rest_api_init', 'lti_register_connectivity_endpoints');
